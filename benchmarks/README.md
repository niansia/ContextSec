...
The v0.4 external-label template is `external-review-template.json`, governed by `external-review.schema.json`. It intentionally contains no cases. A valid completed artifact requires frozen license/sampling provenance, two non-implementer reviewers with expertise classes, raw labels for every pack, separate non-implementer adjudication, and full commit IDs; `scripts/contextsec.py external-review` reports confusion counts, prevalence, agreement, and κ without erasing disagreement.

`holdout-predictions-template.json` keeps post-freeze tool output separate. `scripts/contextsec.py evaluate-holdout <labels> <predictions>` validates the embedded Profiles and model provenance, then reports required/candidate metrics, false-required count, safety-critical recall, exact-set accuracy, and per-pack/framework/support-class results. ContextSec ships these contracts, not fabricated independent labels.
