...
ContextSec keeps four evidence classes separate so a regression pass cannot be presented as general accuracy.
...
| `profile-cases.json` | 36 inline repositories split into 20 development and 16 frozen evaluation cases | none |
| `mutations.json` | 8 baseline/mutant pairs, one for every published deterministic checker shape | none |
...
The mutation score counts a mutation as killed only when the target finding is absent in the baseline, appears exactly once in the mutant with its expected status, and binds the expected control IDs. It covers eight checker shapes, not 116 controls.
